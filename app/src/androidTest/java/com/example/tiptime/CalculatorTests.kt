...
class CalculatorTests {
    ...
    fun calculate_18_percent_tip() {
        onView(withId(R.id.cost_of_service_edit_text)).perform(typeText("60.00"))
    ...
    fun calculate_15_percent_tip(){
        ...
        onView(withId(R.id.cost_of_service_edit_text)).perform(typeText("150"))
        onView(withId(R.id.round_up_switch)).perform(click())
        onView(withId(R.id.calculate_button)).perform(click())
        onView(withId(R.id.tip_result)).check(matches(withText(containsString("22.50"))))
    }
}